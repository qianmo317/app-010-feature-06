...
import { createExportPanel } from '../utils/export-image';
...
function renderResults(container: HTMLElement, results: PickResult[], elapsed: number) {
  ...
  const exportPanel = createExportPanel(results);
  exportBtn.addEventListener('click', () => {
    const willShow = exportPanel.hidden;
    exportPanel.hidden = !willShow;
    exportBtn.textContent = willShow ? '收起导出设置' : '导出吉日清单';
    if (willShow) exportPanel.scrollIntoView({ behavior: 'smooth', block: 'start' });
  });
